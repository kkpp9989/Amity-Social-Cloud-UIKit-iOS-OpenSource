...
//  one-ktb
//
//  Created by Kanyanee Promma on 13/2/2567 BE.
//

import Foundation
import UIKit

public final class AmityFeedView: UIView, AmityRefreshable {
    ...
    private var viewController: AmityViewController
...
//        super.viewWillAppear(animated)
    ...
    public static func make(feedType: AmityPostFeedType) -> AmityFeedView {
        ...
        let vc = AmityFeedView(viewController: AmityFeedViewController())
    ...
    private func setupProtocolHandler() {
        postHeaderProtocolHandler = AmityPostHeaderProtocolHandler(viewController: self.viewController)
        postHeaderProtocolHandler?.delegate = self
        
        postFooterProtocolHandler = AmityPostFooterProtocolHandler(viewController: self.viewController)
        ...
        postPostProtocolHandler?.viewController = self.viewController
    ...
    private func setupReactionPicker() {
        ...
        self.addSubview(reactionPickerView)
...
extension AmityFeedView: AmityPostTableViewDelegate {
...
extension AmityFeedView: AmityPostTableViewDataSource {
...
extension AmityFeedView: AmityFeedScreenViewModelDelegate {
...
extension AmityFeedView: AmityPostHeaderProtocolHandlerDelegate {
...
extension AmityFeedView: AmityPostProtocolHandlerDelegate {
...
extension AmityFeedView: AmityPostFooterProtocolHandlerDelegate {
    
    func footerProtocolHandlerDidPerformView(_ handler: AmityPostFooterProtocolHandler, view: UIView) {
        let likeButtonFrameInSuperview = view.convert(view.bounds, to: self)
...
extension AmityFeedView: AmityPostPreviewCommentDelegate {
    ...
    public func didPerformAction(_ cell: AmityPostPreviewCommentProtocol, action: AmityPostPreviewCommentAction) {
        ...
        case .tapAvatar(let comment):
            AmityEventHandler.shared.userDidTap(from: self.viewController, userId: comment.userId)
        ...
        case .tapReply:
            AmityEventHandler.shared.postDidtap(from: self.viewController, postId: post.postId, pollAnswers: pollAnswers)
        ...
        case .tapOnMention(let userId):
            AmityEventHandler.shared.userDidTap(from: self.viewController, userId: userId)
        case .tapOnHashtag(keyword: let keyword, count: let count):
            AmityEventHandler.shared.hashtagDidTap(from: self.viewController, keyword: keyword, count: count)
        case .tapCommunityName(post: let post): // [Custom for ONE Krungthai] Add tap to community for moderator user in official community action
                AmityEventHandler.shared.communityDidTap(from: self.viewController, communityId: post.targetCommunity?.communityId ?? "")
        }
    }
    ...
    private func handleCommentOption(comment: AmityCommentModel) {
        ...
        let editOption = TextItemOption(title: AmityLocalizedStringSet.PostDetail.editComment.localizedString) { [weak self] in
            ...
            strongSelf.viewController.present(nvc, animated: true, completion: nil)
        }
...
extension AmityFeedView: IndicatorInfoProvider {

//
//  AmityPendingMembersViewController.swift
//  AmityUIKit
//
...
public class AmityPendingMembersViewController: AmityViewController {

    // MARK: - IBOutlet Properties
    @IBOutlet private var tableView: AmityPostTableView!
    
    // MARK: - Properties
    private var screenViewModel: AmityPendingMembersScreenViewModelType!
    private let headerView = AmityPendingMembersHeaderView()
    private let emptyView = AmityPendingMembersEmptyView()
    ...
    public static func make(communityId: String) -> AmityPendingMembersViewController {
        let viewModel = AmityPendingMembersScreenViewModel(communityId: communityId)
        let vc = AmityPendingMembersViewController(nibName: AmityPendingMembersViewController.identifier, bundle: AmityUIKitManager.bundle)
    ...
    private func setupTableView()  {
        ...
        tableView.register(cell: AmityPendingMembersActionTableViewCell.self)
...
extension AmityPendingMembersViewController: AmityPostTableViewDataSource {
    ...
    func tableView(_ tableView: AmityPostTableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        (cell as? AmityPendingMembersActionCellProtocol)?.delegate = self
...
extension AmityPendingMembersViewController: AmityPendingMembersScreenViewModelDelegate {
    ...
    func screenViewModel(_ viewModel: AmityPendingMembersScreenViewModelType, didGetMemberStatusCommunity status: AmityMemberStatusCommunity) {
        switch status {
        case .admin, .member:
            viewModel.action.getPendingMembers()
        case .guest:
            break
        }
...
extension AmityPendingMembersViewController: AmityPostHeaderDelegate {
...
extension AmityPendingMembersViewController: AmityPendingMembersActionCellDelegate {
...
extension AmityPendingMembersViewController: AmityPostDelegate {
